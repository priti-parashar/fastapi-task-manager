...
def get_tasks(current_user: dict = Depends(get_current_user)):
    ...
    tasks_data = [{"id": t.id, "title": t.title, "description": t.description} for t in tasks]
...
def update_task(task_id: int, title: str, description: str = None, current_user: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        user = db.query(User).filter(User.username == current_user["sub"]).first()
